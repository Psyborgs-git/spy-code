...
const claude_code_adapter_1 = require("@spy-code/claude-code-adapter");
...
async function activate(context) {
    ...
    // Initialize adapter
    adapter = new claude_code_adapter_1.ClaudeCodeAdapter({
...
async function registerSkills() {
    ...
    try {
        ...
        await fs.writeFile(path.join(skillsDir, 'spy-code-search.json'), JSON.stringify(searchSkill, null, 2));
        ...
        await fs.writeFile(path.join(skillsDir, 'spy-code-semantic-search.json'), JSON.stringify(semanticSearchSkill, null, 2));
        // Get node skill
        const getNodeSkill = {
        ...
        await fs.writeFile(path.join(skillsDir, 'spy-code-get-node.json'), JSON.stringify(getNodeSkill, null, 2));
        // Callers skill
        const callersSkill = {
        ...
        await fs.writeFile(path.join(skillsDir, 'spy-code-callers.json'), JSON.stringify(callersSkill, null, 2));
        // Callees skill
        const calleesSkill = {
        ...
        await fs.writeFile(path.join(skillsDir, 'spy-code-callees.json'), JSON.stringify(calleesSkill, null, 2));
...
`;
        await fs.writeFile(path.join(workflowsDir, 'analyze-function.md'), analyzeFunctionWorkflow);
...
`;
        await fs.writeFile(path.join(workflowsDir, 'impact-analysis.md'), impactAnalysisWorkflow);
...
`;
        await fs.writeFile(path.join(workflowsDir, 'code-review.md'), codeReviewWorkflow);
