...
export function registerCommands(
  ...
  const searchCommand = vscode.commands.registerCommand(
    ...
    async () => {
      ...
      if (query) {
        try {
          eventBus.emit(EventType.SEARCH_STARTED, { query, options: {} });
          const results = await cliBridge.search(query);
          eventBus.emit(EventType.SEARCH_COMPLETED, { query, results });
          ...
          if (selected) {
            await adapter.goToDefinition(
              selected.result.node.filePath,
              selected.result.node.startLine
            );
          }
        } catch (error) {
          eventBus.emit(EventType.SEARCH_FAILED, { query, error });
    ...
    async () => {
      ...
      if (word) {
        try {
          ...
          if (results.length > 0) {
            await adapter.goToDefinition(
              results[0].node.filePath,
              results[0].node.startLine
            );
          } else {
  ...
  const reindexCommand = vscode.commands.registerCommand(
    ...
    async () => {
      ...
      if (full) {
        try {
          eventBus.emit(EventType.INDEX_STARTED, { full: full === 'Full' });
          await vscode.window.withProgress(
            ...
            async () => {
              ...
              eventBus.emit(EventType.INDEX_COMPLETED, { stats });
              vscode.window.showInformationMessage(
                `Reindex complete: ${stats.nodeCount} nodes, ${stats.edgeCount} edges, ${stats.fileCount} files`
              );
            }
          );
        } catch (error) {
          eventBus.emit(EventType.INDEX_FAILED, { error });
...
function generateCallGraphText(
  node: any,
  callers: any[],
  callees: any[]
): string {
